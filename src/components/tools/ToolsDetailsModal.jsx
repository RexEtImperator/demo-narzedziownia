...
const ToolsDetailsModal = ({
  ...
  return (
    ...
    >
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow-xl w-full max-w-5xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
                          ...
                          <div key={`${it.id || idx}-${it.employee_id || idx}`} className="flex flex-col gap-0.5">
                            <div className="flex items-center justify-between">
                              <div className="text-md text-slate-700 dark:text-slate-200 sharp-text">
                                ...
                                >
                                  {brand ? ` [${brand}]` : ''} {name || t('tools.details.unknownUser')}
                                </button>
                              </div>
          ...
          {['detektory'].includes(String(selectedTool.category || '').trim().toLowerCase()) && (
            <div className="mt-6 pt-6 border-t border-slate-200 dark:border-slate-700">
              <ToolsDetectorsItemsTable toolId={selectedTool.id} t={t} canManage={canManageTools} highlightSku={highlightSku} />
            </div>
          )}
        </div>
      </div>
